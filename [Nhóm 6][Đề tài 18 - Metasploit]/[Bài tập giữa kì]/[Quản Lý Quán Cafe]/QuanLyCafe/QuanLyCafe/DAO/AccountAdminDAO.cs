...
namespace QuanLyCafe.DAO
{
    class AccountAdminDAO
    {
      ...
      public AccountAdmin GetAccoutbyusername(string username)
        {
            DataTable data = DataProvider.Instance.ExecuteQuery("select * from TBL_TAIKHOAN where TENDANGNHAP = " + username);
            foreach(DataRow item in data.Rows)
            {
                return new AccountAdmin(item);
            }
            return null;
        }
        ...
        public bool insertAccountAdmin(string username, string password, string quyen)
        {
            string query = "USP_InsertAccountAdmin @username , @password , @quyen";
            int res = DataProvider.Instance.ExecuteNonQuery(query, new object[] { username, encode(password), quyen });
            return res > 0;
        }
